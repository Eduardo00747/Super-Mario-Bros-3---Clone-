...
public class Box_Multiple_Coin : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.CompareTag("Player") && !isRecuando && !mecanicaExecutada && vezesExecutadas < 5)
        {
    ...
    IEnumerator ResetPositionAfterDelay()
    {
        ...
        // Verifica se é a quinta execução antes de ativar a animação
        if (vezesExecutadas == 5)
        {
            animator.SetBool("Break_Box", true); // Ativa a animação
        }

        // Reseta a flag de execução da mecânica para permitir a próxima execução
        mecanicaExecutada = false;
    }
}
